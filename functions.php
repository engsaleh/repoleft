...
function frannawp_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'frannawp' ),
		)
	);
...
function frannawp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'frannawp_content_width', 640 );
}
...
function frannawp_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'frannawp' ),
			'id'            => 'sidebar-1',
...
function frannawp_scripts() {
	wp_enqueue_style( 'frannawp-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'frannawp-style', 'rtl', 'replace' );

	wp_enqueue_script( 'frannawp-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
